...
function rzDataTable() {
  return {
    ...
    computeCellClass(columnConfig) {
      const alignClass = columnConfig?.align === "center" ? "text-center" : columnConfig?.align === "end" ? "text-right" : "text-left";
    ...
    renderPaginationState() {
      ...
      this.$refs.pageIndicator.textContent = pageCount > 0 ? `Page ${pageIndex + 1} of ${pageCount}` : "Page 0 of 0";
    },
    ...
    getOrderedSelectedKeys() {
      return this.table.getRowModel().rows.map((row) => row.id).filter((key) => !!this.state.rowSelection[key]);
    },
...
//# sourceMappingURL=table-runtime-CjgOEIv0.js.map
