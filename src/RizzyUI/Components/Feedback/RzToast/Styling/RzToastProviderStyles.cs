...
public static class RzToastProviderStyles
{
    ...
    public static readonly TvDescriptor<RzComponent<RzToastProviderSlots>, RzToastProviderSlots> DefaultDescriptor = new(
        ...
        variants: new()
        {
            ...
            {
                [ToastStatus.Default] = new() { [s => s.Toast] = "border-border bg-background text-foreground", [s => s.Title] = "text-foreground", [s => s.IconContainer] = "text-muted-foreground", [s => s.IconPulse] = "bg-muted/50", [s => s.ProgressIndicator] = "bg-muted-foreground", [s => s.LoadingIndicator] = "text-muted-foreground" },
