...
async function resolveRenderable(payloadOrValue) {
    ...
    const value =
        typeof payload.def === 'function'
            ? await payload.def(payload.ctx)
            : payload.def;
...
function applyHtml(el, html, payload, options) {
    ...
    const finalHtml =
        typeof options.sanitizeHtml === 'function' && payload
            ? options.sanitizeHtml(html, payload)
            : html;
...
function applyNode(el, node, options) {
    ...
    const finalNode =
        options.cloneNodeResults === false
            ? node
            : node.cloneNode(true);
